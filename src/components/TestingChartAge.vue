  ...
  data: function () {
    return {
      ...
      chartOptions: {
        ...
        title: {
          text: 'Total Number of Voters by Voter Age',
          align: 'center'
        },
        grid: {
          borderColor: '#e7e7e7',
        ...
        yaxis: {
          title: {
            text: 'Total Number of Early Voters'
          },
          min: 0,
          max: 40
        },
    ...
    axios.get('https://hevm-backend.herokuapp.com/api/testing_data_count/', {
      ...
      .then((response) => {
        ...
        for (var i = 0; i < this.agetotals.length; i++) {
          arr.push(this.agetotals[i].age__count)
        }
        this.agepcts = arr
      }
      )
